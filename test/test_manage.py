...
def test_fault_localization_success(monkeypatch, tmp_path):
    ...
    dummy_ranked_methods_abs = [("dummy/path/dummy1.py", "DummyClass", "dummy_method", "score")]
    ...
    monkeypatch.setattr("app.manage.sbfl.collate_results", lambda lines, files: dummy_ranked_ranges_abs)
    monkeypatch.setattr("app.manage.sbfl.map_collated_results_to_methods", lambda ranges: dummy_ranked_methods_abs)
...
def test_reproduce_success(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("app.manage.agent_reproducer.generator", lambda issue: dummy_generator(issue))
