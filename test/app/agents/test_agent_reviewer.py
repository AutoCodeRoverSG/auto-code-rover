...
def test_extract_review_result_valid():
    ...
    assert review.to_json() == data, "The to_json output does not match the original data"
...
def test_run_with_retries_valid_first_attempt():
    ...
    gen = run_with_retries(issue_statement, test_str, patch_str,
                           orig_stdout, orig_stderr, patched_stdout, patched_stderr,
                           retries=3)
    ...
    assert any(SYSTEM_PROMPT in msg["content"] for msg in thread.messages if msg["role"] == "system")
...
def test_run_successful():
    ...
    review, thread = run(issue_statement, test_content, patch_content, dummy_repro, dummy_repro)
    assert review is not None
    assert review.patch_decision == ReviewDecision.YES
