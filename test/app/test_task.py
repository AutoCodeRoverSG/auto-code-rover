...
class DummyTask(BaseDummyTask):
    def __init__(self, project_path="dummy_project", issue="dummy issue", env_name="dummy_env"):
...
def test_plain_task_getters_and_reset(monkeypatch, tmp_path):
    ...
    task = PlainTask(commit_hash=commit, local_path=str(local_dir), problem_statement=problem)
...
def dummy_swe_task(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(apputils, "run_string_cmd_in_conda", fake_run_string_cmd_in_conda)
...
def test_swe_task_getters_and_setup(dummy_swe_task, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(apputils, "repo_reset_and_clean_checkout", lambda commit: calls.append(commit))
...
def test_swe_task_validate(monkeypatch, dummy_swe_task, tmp_path):
    ...
    monkeypatch.setattr(dummy_swe_task, "_run_test_suite_for_regression_docker", fake_run_test_suite_docker)
